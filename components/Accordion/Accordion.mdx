...
export const Accordion = ({ children, icon, iconColor, title }) => {
  ...
  return (
    <details className="border border-gray-200 rounded-md" onToggle={() => setIsOpen(!isOpen)}>
      <summary
        className={`
          flex items-center justify-start w-full p-2 text-base font-medium cursor-pointer rounded-md
