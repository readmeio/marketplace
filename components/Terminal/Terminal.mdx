...
export const Terminal = ({ children }) => {
  ...
  const copyToClipboard = (text, index) => {
    navigator.clipboard.writeText(text.trim().slice(1).trim()).then(() => {
        ...
        {lines.map((line, index) => {
          ...
          return (
            ...
            >
              ...
              <span className={isInput ? ` ${isInput && hoveredIndex === index ? 'text-white' : 'text-gray-200'}` : 'text-indigo-300'}>
                {content}
              </span>
              {isInput && hoveredIndex === index && (
                <span
